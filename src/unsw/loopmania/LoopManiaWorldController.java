...
public class LoopManiaWorldController {
    ...
    @FXML
    private Label characterGoldContent;

    @FXML
    private Label characterEXPContent;
    ...
    private int emptyEquipmentSlotNum;

    private int totalUnEquipmentSlotNum;
    ...
    public void initialize() {
        ...
        GlobalConfig.EntityImagePath.forEach((name, path) -> imageMap.put(name, new Image(new File(path).toURI().toString())));
        ...
        totalUnEquipmentSlotNum = emptyEquipmentSlotNum = unequippedInventory.getChildren().size();
        ...
        // Load character
        onLoad(GlobalConfig.character);
        // Load enemies
        world.getEnemies().forEach(this::onLoad);
        // Load buildings
        world.getBuildings().forEach(this::onLoad);
        // Load equipments
        world.getUnequippedInventoryItems().forEach(this::onLoadUnequipped);
        // Load Cards
        world.getCards().forEach(this::onLoad);

        world.cycleNumProperty().addListener(new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> observableValue, Number number, Number t1) {
                world.getBuildings().forEach(building -> onCycle(building));
            }
        });

        bindCharacterEvent(GlobalConfig.character);
    }

    private void onCycle(BaseBuilding building) {
        BaseEnemy enemy = building.onCycle(world.getCycleNum());
        if (enemy != null) {
    ...
    // Allows a Screenshot to be taken
    @FXML
    public void handleScreenshot(ActionEvent event) throws IOException {
        WritableImage image = anchorPaneRoot.snapshot(new SnapshotParameters(), null);
        File file = new File("Screenshot.png");
        ImageIO.write(SwingFXUtils.fromFXImage(image, null), "png", file);
    }

    // Sets the Shops Screen
    ...
    private void bindCharacterEvent(Character character) {
        System.out.println("Bind character event");
        character.shopOpen().addListener((observable, oldValue, newValue) -> {
            if (!oldValue && newValue) {
                pause();
                shopScreen.start();
                character.shopNow(false);
            }
        });
        character.shouldExist().addListener((observable, oldValue, newValue) -> {
            if (oldValue && !newValue && character.getHP() <= 0) {
                lostScreen.start();
            } else if (oldValue && !newValue && character.reachGoal()) {
                winScreen.start();
            }
        });
        character.HPProperty().addListener(new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                characterHPBar.setProgress(character.getHP() / (double) GlobalConfig.GameStatics.CharacterInitHP);
            }
        });
        character.expProperty().addListener(new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                characterEXPContent.setText(String.valueOf(character.getExp()));
            }
        });
        character.goldProperty().addListener(new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                characterGoldContent.setText(String.valueOf(character.getGold()));
            }
        });
    }
    ...
    public void startTimer() {
        ...
        timeline = new Timeline(new KeyFrame(Duration.seconds(0.3), event -> {
            ...
            List<BaseEnemy> newEnemies = world.possiblySpawnEnemies();
            if (newEnemies != null) {
                for (BaseEnemy newEnemy : newEnemies) {
                    onLoad(newEnemy);
                }
            }
...
//        BaseCard vampireCastleCard = world.loadVampireCard();
//        onLoadCard(vampireCastleCard);
//    }
//
//    /**
...
//        BaseEquipment sword = world.addUnequippedSword();
//        onLoadEquipment(sword);
//    }
     ...
     */
    private void reactToEnemyDefeat(BaseEnemy enemy) {
        // react to character defeating an enemy
        // in starter code, spawning extra card/weapon...
        // TODO = provide different benefits to defeating the enemy based on the type of enemy
        System.out.printf("Enemy %s is defeated. Get rewards\n%n", enemy.getType());
    }

    /**
     * load a entity into the GUI.
     * Particularly, we must connect to the drag detection event handler,
     * and load the image into the cards GridPane.
     *
     * @param card
     */
    private void onLoad(BaseCard card) {
        ImageView view = new ImageView(imageMap.get(card.getType()));
     ...
     * @param equipment
     */
    protected void onLoadUnequipped(BaseEquipment equipment) {
        ImageView view = new ImageView(imageMap.get(equipment.getType()));
        addDragEventHandlers(view, DRAGGABLE_TYPE.ITEM, unequippedInventory, equippedItems);
        trackPosition(equipment, view);
        unequippedInventory.getChildren().add(view);
    }

    private void onLoadEquipped(BaseEquipment equipment) {
        ImageView view = new ImageView(imageMap.get(equipment.getType()));
     ...
     * @param enemy
     */
    private void onLoad(BaseEnemy enemy) {
        ImageView view = new ImageView(imageMap.get(enemy.getType()));
        enemy.shouldExist().addListener((observable, oldValue, newValue) -> {
            if (oldValue && !newValue) {
                reactToEnemyDefeat(enemy);
                squares.getChildren().remove(view);
                Pair<BaseCard, BaseEquipment> rewards = world.getEnemyRewards();
                onLoad(rewards.getValue0());
                onLoadUnequipped(rewards.getValue1());

            }
        });
        trackPosition(enemy, view);
        squares.getChildren().add(view);
//        printThreadingNotes("onLoadEnemy", String.format("Enemy %s is loaded to (%d, %d)", enemy.getType(), enemy.getX(), enemy.getY()));
    }


    private void onLoad(Character character) {
        ImageView view = new ImageView(imageMap.get(character.getType()));
        trackPosition(character, view);
        squares.getChildren().add(view);
    }
     ...
     * @param building
     */
    private void onLoad(BaseBuilding building) {
        ImageView view = new ImageView(imageMap.get(building.getType()));
        trackPosition(building, view);
        squares.getChildren().add(view);
    }
                ...
                if (currentlyDraggedType == draggableType) {
                    ...
                    if (node != targetGridPane && db.hasImage()) {
                        ...
                        switch (draggableType) {
                            case CARD:
                                BaseBuilding newBuilding = convertCardToBuildingByCoordinates(nodeX, nodeY, x, y);
                                if (newBuilding != null) {
                                ...
                                break;
                            case ITEM:
                                LoopManiaWorld.ChangeEquipmentType type;
                                if (sourceGridPane == unequippedInventory) {
                                    type = LoopManiaWorld.ChangeEquipmentType.equip;
                                } else {
                                    type = LoopManiaWorld.ChangeEquipmentType.unEquip;
                                }
                                BaseEquipment equipment = changeEquipment(nodeX, nodeY, x, y, type);
                                if (equipment != null) {
                                    removeDraggableDragEventHandlers(draggableType, targetGridPane);
//                                    removeItemByCoordinates(nodeX, nodeY);
                                    equipment.setX(x);
                                    equipment.setY(y);
                                    if (type == LoopManiaWorld.ChangeEquipmentType.equip) {
                                        System.out.println("Equip "+equipment.getType());
                                        onLoadEquipped(equipment);
                                    } else {
                                        System.out.println("Unequip "+equipment.getType());
                                        onLoadUnequipped(equipment);
                                    }
                                }else{

                                    System.out.println("Change equipment failed");
                                }
...
//                        printThreadingNotes("DRAG DROPPED ON GRIDPANE HANDLED", "");
     ...
     */
    private BaseBuilding convertCardToBuildingByCoordinates(int cardNodeX, int cardNodeY, int buildingNodeX, int buildingNodeY) {
        return world.convertCardToBuildingByCoordinates(cardNodeX, cardNodeY, buildingNodeX, buildingNodeY);
    }

    private BaseEquipment changeEquipment(int fromX, int fromY, int toX, int toY, LoopManiaWorld.ChangeEquipmentType type) {
        return world.changeEquipment(fromX, fromY, toX, toY, type);
    }
...
//                }
     ...
     */
    @FXML
    private void switchToGamePaused() throws IOException {
        // TODO = possibly set other menu switchers
        pause();
     ...
     */
    private void printThreadingNotes(String currentMethodLabel, String message) {
        System.out.println("\n###########################################");
