...
//        assert indexInPath != -1;
        ...
        switch (type) {
            case GlobalConfig.BuildingName.HeroCastle:
                assert indexInPath != -1;
                GlobalConfig.character = world.getCharacterFactory().createCharacter(new PathPosition(indexInPath, orderedPath));
                world.setCharacter();
//                onLoad(character);
                BaseBuilding heroCastle = world.getBuildingFactory().createHeroCastle(new SimpleIntegerProperty(orderedPath.get(indexInPath).getValue0()), new SimpleIntegerProperty(orderedPath.get(indexInPath).getValue1()));
                world.addEntity(heroCastle);
                break;
            case GlobalConfig.BuildingName.ZombiePit:
            case GlobalConfig.BuildingName.VampireCastle:
            case GlobalConfig.BuildingName.Tower:
            case GlobalConfig.BuildingName.Campfire:
                assert indexInPath == -1;
                BaseBuilding building = world.getBuildingFactory().createBuilding(new SimpleIntegerProperty(x), new SimpleIntegerProperty(y), type);
                world.addEntity(building);
                break;
            case GlobalConfig.BuildingName.Village:
            case GlobalConfig.BuildingName.Trap:
            case GlobalConfig.BuildingName.HealthPotion:
            case GlobalConfig.BuildingName.Gold:
                assert indexInPath != -1;
                BaseBuilding onPathBuilding = world.getBuildingFactory().createBuilding(new SimpleIntegerProperty(x), new SimpleIntegerProperty(y), type);
                world.addEntity(onPathBuilding);
                break;
            case GlobalConfig.EnemyName.Slug:
            case GlobalConfig.EnemyName.Zombie:
            case GlobalConfig.EnemyName.Vampire:
            case GlobalConfig.EnemyName.Doggie:
            case GlobalConfig.EnemyName.ElanMuske:
                assert indexInPath != -1;
                BaseEnemy enemy = world.getEnemyFactory().createEnemy(new PathPosition(indexInPath, orderedPath), type);
                world.addEntity(enemy);
                break;
            case GlobalConfig.EquipmentName.Sword:
            case GlobalConfig.EquipmentName.Stake:
            case GlobalConfig.EquipmentName.Staff:
            case GlobalConfig.EquipmentName.Armour:
            case GlobalConfig.EquipmentName.Shield:
            case GlobalConfig.EquipmentName.Helmet:
            case GlobalConfig.EquipmentName.TheOneRing:
            case GlobalConfig.EquipmentName.TreeStump:
            case GlobalConfig.EquipmentName.Anduril:
            case GlobalConfig.EquipmentName.DoggieCoin:
                BaseEquipment equipment = world.getEquipmentFactory().createEquipment(new SimpleIntegerProperty(x), new SimpleIntegerProperty(y), type);
                world.addEntity(equipment);
                break;
            case GlobalConfig.CardName.VampireCastle:
            case GlobalConfig.CardName.ZombiePit:
            case GlobalConfig.CardName.Tower:
            case GlobalConfig.CardName.Village:
            case GlobalConfig.CardName.Barracks:
            case GlobalConfig.CardName.Trap:
            case GlobalConfig.CardName.Campfire:
                BaseCard card = world.getCardFactory().createCard(new SimpleIntegerProperty(x), new SimpleIntegerProperty(y), type);
